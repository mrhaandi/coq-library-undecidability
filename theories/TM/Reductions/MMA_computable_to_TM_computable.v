...
Require Import Undecidability.Shared.Libs.PSL.FiniteTypes.CompoundFinTypes.
...
Definition state' : finType := finType_CS (sum (Fin.t maxState) unit).

Definition toState (p : nat) : state' :=
  inl (if le_lt_dec maxState p is right H then Fin.of_nat_lt H else Fin.F1).

Definition trans' : state' * Vector.t (option Σ) n -> state' * Vector.t (option Σ * move) n :=
  fun '(plr, bs) =>
    match plr with
    | inr _ => 
        if Vector.fold_right (fun b acc => if b is None then acc else true) bs false then (plr, Vector.const (None, Rmove) n)
        else (toState 1, TM_facts.nop_action)
    | inl p =>
      match sval (Fin.to_nat p) with
      | 0 => (plr, TM_facts.nop_action)
      | S m' =>
          match nth_error M m' with
          | None => (plr, TM_facts.nop_action)
          | Some (mm_inc i) =>
...
Definition halt' (qlr : state') :=
  match qlr with
  | inr _ => false
  | inl q => Fin.eqb (@Fin.F1 (length M)) q
  end.
...
Definition state'_to_nat (plr : state') :=
  match plr with
  | inr _ => 0
  | inl p => sval (Fin.to_nat p)
  end.

(*
Lemma read_instr {p instr} : (p, [instr]) <sc (1, M) ->
  exists q, state'_to_nat (toState p) = S q /\ nth_error M q = Some instr /\ p = S q.
Proof.
  rewrite /toState /state'_to_nat. case: (le_lt_dec maxState p).
...
*)
Lemma read_instr {p instr} : (p, [instr]) <sc (1, M) ->
  exists p' q, (toState p) = inl p' /\ sval (Fin.to_nat p') = S q /\ nth_error M q = Some instr /\ p = S q.
Proof.
  rewrite /toState. case: (le_lt_dec maxState p).
  { move=> ? [?] [?] [/(f_equal (@length _))].
    rewrite app_length /=. by lia. }
  move=> Hp [l] [r] [HM H'p]. eexists. exists (length l).
  split; [done|].
  by rewrite Fin.to_nat_of_nat /= HM nth_error_app2 ?Nat.sub_diag.
Qed.
...
From Undecidability Require Import
  MinskyMachines.Util.MMA_computable
  TM.Util.TM_computable.


Lemma MMA_computable_to_TM_computable k (R : Vector.t nat k -> nat -> Prop) :
